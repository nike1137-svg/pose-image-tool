...
참조 사진의 **자세만** 뽑아 다른 인물·장면으로 옮기는 도구다. 실험은 **세 번** 했다.

| | 무엇을 했나 | 무엇을 알았나 |
|---|---|---|
| **1차** | 포즈 2종 × 프롬프트 3종 + 강도 3단계, 총 8장. 시드 규칙으로 재현 검증, 관절 좌표로 자세 일치도 측정 | 프롬프트를 바꿔도 자세는 유지된다 — 참조 사진에서 관절만 넘어간다 |
| **2차** [(재실험)](#재실험--대조군과-시드-고정) | 조건을 **끈** 대조군 추가, 시드를 고정하고 강도만 변경, 시드만 바꿔 5장 비교 | **1차 결론 다섯 개가 뒤집혔다.** 원인은 대조군 없음 · 두 값을 동시에 바꿈 · 한 장으로 일반화 |
| **3차** [(조건에 무엇이 담겼나)](#조건에-무엇이-담겨-있나) | 관절 진단, 상반신만 조건, 프롬프트와 정면 충돌, 충돌+강도 4단계 | 조건은 자세가 아니라 **2D 좌표**를 통제한다. 좌표를 어떻게 읽을지는 강도가 정한다 |

1차 기록은 **지우지 않고** 남겨두고 정정 표를 붙였다. 결과보다 틀렸다가 고친 과정이 남는 게 많아서다.

- 처음 만든 기록 → [테스트 결과](#테스트-결과)
- 되돌린 기록 → [재실험 — 대조군과 시드 고정](#재실험--대조군과-시드-고정)
- 조건 자체를 캐물은 기록 → [조건에 무엇이 담겨 있나](#조건에-무엇이-담겨-있나)
- 아직 확인 못 한 것 → [손 조건 미결](#손-조건--켤-수-있었다-그리고-미결), 더 큰 모델에서도 같은지
...
> ⚠️ **먼저 읽어주세요.** 이 절은 처음 실험한 기록이다. 그 뒤 [재실험](#재실험--대조군과-시드-고정)과 [조건에 무엇이 담겨 있나](#조건에-무엇이-담겨-있나)에서 다시 확인한 결과 **여기 적힌 결론 여러 개가 뒤집혔다.** 기록은 지우지 않고 남겨두었으니 두 절의 정정 표를 함께 봐주세요.
...
조건이 자세를 담고 있다고 생각해 왔는데 확인한 적이 없었다. 세 가지가 나왔다.
...
관절 진단을 붙여 보니 전신은 18/18, 상반신만 자른 것은 14/18 이었다. 빠진 것은 무릎·발목 네 개다. 그리고 14/18 은 전부 검정이 아니므로 5번 셀의 실패 처리를 그냥 통과한다. **형식은 통과, 내용은 절반이다.**

**둘. 프롬프트로는 자세를 못 바꾼다.**

![충돌](samples/conflict_prompt.png)

가운데는 프롬프트에 `sitting cross-legged on the ground` 를 넣은 것이다. 앉지 않았다. 자세를 바꾸려면 참조 사진을 바꿔야 한다.

**셋. 강도는 "어디까지 따르나"를 정한다.**
...
0.6 이 핵심이다. 관절 좌표는 스켈레톤과 잘 맞는데 사람은 누워 있다. 스켈레톤에는 깊이가 없어서, 정면에서 본 서 있는 사람과 위에서 본 누운 사람의 2D 좌표가 거의 같다. 모델은 조건을 어기지 않고 사람을 눕혔다.

그러면 자세오차와 PCK 는 이 경우를 "잘 됐다"고 판정한다. **좌표만 재는 지표는 좌표가 무엇을 뜻하는지 모른다.**

셀 단위 기록과 나머지 관찰은 노트 14번 절에 있다.
...
- **전신이 안 보이면 그 부분만 조건에서 빠진다.** 처음에는 "쓸 수 없다"고 적었는데 사실이 아니었다. 상반신만 잘라도 검출은 되고 14/18 이 잡힌다. 안 보인 부위만 조건에서 사라지고 결과에서도 그 부위가 빠진다.
- **5번 셀의 실패 처리는 전부 실패만 잡는다.** 스켈레톤이 전부 검정인 경우만 걸러낸다. 14/18 같은 부분 조건은 통과하고 "관절 추출 성공"이 찍힌다. 오전 배치에서 결측값이 문자열로 흘러들어간 것과 같은 구조다.
...
- **조건이 통제하는 것은 2D 좌표다.** 스켈레톤에는 깊이가 없다. 정면에서 본 서 있는 사람과 위에서 본 누운 사람은 관절의 2D 좌표가 거의 같아서, 모델은 조건을 어기지 않고 사람을 눕힐 수 있다. 자세를 확실히 지정하려면 강도를 충분히 올려야 하고, 그러면 카메라 각도까지 조건이 가져간다.
- **자세오차·PCK 는 좌표만 재고 좌표의 해석은 못 잡는다.** 누운 사람이 낮은 오차를 받을 수 있다. 지표를 볼 때 그림을 같이 봐야 한다.
- **프롬프트로는 자세를 바꿀 수 없다.** `sitting cross-legged` 를 넣어도 무시됐다. 자세를 바꾸려면 참조 사진을 바꿔야 한다.
- **더 큰 모델에서도 같은지는 확인하지 않았다.** 위 관찰은 `SD1.5 + control_v11p_sd15_openpose` 조합에서 얻은 것이다. 모델 급수·어댑터의 학습 내용·스켈레톤의 2D 성질 중 무엇이 얼마나 기여했는지 이 실험으로는 가릴 수 없다. SDXL 로 같은 충돌 실험을 하면 모델 축을 분리할 수 있지만, 무료 T4 는 시스템 RAM 이 12.7GB 뿐이라 로딩이 아슬아슬해 하지 않았다.
